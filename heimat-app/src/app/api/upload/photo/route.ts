...
/**
 * Property photo upload presigned URL using GCS Interop (S3-compatible) API.
...
export async function POST(req: Request) {
  try {
    ...
    if (!bucket || !accessKeyId || !secretAccessKey) {
      const key = `${landlordId ?? "anon"}/properties/${Date.now()}-${fileName}`;
    ...
    if (isDirectUpload && file) {
      ...
      if (!uploadRes.ok) {
        ...
        console.error("GCS direct photo upload failed:", errText);
